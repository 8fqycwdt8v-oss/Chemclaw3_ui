...
2. **Provenance awareness** — the UI knows that a chemical number is not just a number: it has a
   method (a GFN2-xTB single point vs a CREST search), a solvent model, an uncertainty, a
   calibration history, and a validity
...
5. `KNOWN_TOOLS` (`shared/events.ts`) and `TOOL_ICON` (`src/components/TracePanel.tsx`) list
   `submit_qm_job` and `get_qm_job_status`, which no longer exist (the real name is
   `get_durable_job_status`), and omit ~35 tools that do. **Resolved for the dead QM names**: they
   are removed, along with `compute_dft_energy` itself, which Chemclaw3 deleted with the whole
   HPC/DFT tier (`D-2026-08-26-semiempirical-is-the-whole-tier`).
...
| US-14 | _…I want a long conformer search to tell me when it fails, not just when it succeeds._                                                            | `JobFailedEvent` on `GET /sessions/{id}/events`                                                                                                                                               | Mirror `job_failed`; a failed-job card                                           | **yes — pure frontend bug**                |
...
- **Method badges.** A GFN2-xTB single point vs a CREST search vs a lookup table vs a surrogate
  model, derived from the tool that produced the value. A chemist should never have to ask which
  one a number came from — and where none of them is accurate enough, that the ceiling is
  semiempirical is itself the answer.
