 ...
 * it is reported and does not fail. Read that list on every run anyway: seven of its rows were real
 * gaps rather than decisions, and nothing but reading it told anyone.
 *
...
if (declared.length === 0) {
  ...
  if (unhandled.length === 0) {
    ok('normalizeEvent admits every member of the ChemclawEvent union', `${declared.length} members`);
  } else {
