 ...
 *
 * Chosen from the use case — the active conversation plus the two most recently used, which covers
 * "I launched a run, moved on, and it finished" — and now checked against the real cap rather than
 * left as a defensible guess. The service's is `service_max_event_streams_per_user`, default **5**
 * (`chemclaw/core/config/service.py`), enforced in `routes/streams.py` alongside a per-pod total.
 * Three fits under five with room to spare, so the number stands; what has changed is that it is
 * now a margin instead of a hope.
 *
 * The margin is thinner than it looks, and this is the part worth knowing. That cap is **per
 * principal, per process, counted across connections** — it does not know what a tab is. Two tabs
 * open on the same account ask for six streams against a cap of five, so the second tab's last
 * stream 429s. Nothing here can see the other tab: the count lives in the pod's memory, and a
 * client-side budget can only bound its own. So the overshoot is real, expected in a
 * two-window workflow, and handled rather than prevented — the 429 path below drops this tab to a
 * single stream, which brings the pair back under the cap. Preventing it properly means one tab
 * holding the streams for all of them (a `BroadcastChannel` leader election), which is a feature,
 * not a constant. It is written down in ISSUES.md instead of being half-built here.
 */
...
async function openStream(
  ...
  while (!controller.signal.aborted) {
    try {
      ...
      // spell would flap against whatever else is holding the cap, and the cost of staying low is
      // one tab watching one conversation instead of three. Down is cheap; oscillating is not.
      if (res.status === 429) {
        consecutive429 += 1;
