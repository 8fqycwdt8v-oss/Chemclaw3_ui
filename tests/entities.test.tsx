...
describe('jobs', () => {
  it('closes a running job when the push-back stream reports it', async () => {
    ...
    await ingest('job:qm-1', { type: 'job_failed', job_id: 'qm-1', reason: 'walltime' });
...
describe('switching conversation', () => {
  ...
  it('does not let one conversation’s selection filter another’s transcript', () => {
    const conversation = (id: string, messageId: string): Conversation => ({
      id,
      sessionId: null,
      title: id,
      createdAt: 0,
      updatedAt: 0,
      contextLost: false,
      messages: [{ id: messageId, role: 'user', text: `asked in ${id}`, at: 0 }],
    });

    // A selection in the first conversation. Its mention list names `a1` and nothing else.
    void store().ingest(C1, 'a1', { type: 'job_started', job_id: 'qm-1', kind: 'qm' });
    ...
    // rendered "No turn in this conversation mentions that yet." over a conversation full of turns.
    render(<MessageList conversation={conversation(C2, 'b1')} />);
    expect(screen.getByText('asked in conversation-2')).toBeTruthy();
    expect(screen.queryByText(/No turn in this conversation mentions/)).toBeNull();
  ...
  it('drops a conversation’s index when the conversation is deleted', () => {
    const chat = useChatStore.getState();
    const id = chat.createConversation();
    void store().ingest(id, 'm1', { type: 'job_started', job_id: 'qm-9', kind: 'qm' });
